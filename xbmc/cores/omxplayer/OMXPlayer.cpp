...
static void UpdatePlexSelectionStream(OMXSelectionStream &s, const CFileItemPtr &part);
...
void COMXSelectionStreams::Update(CDVDInputStream* input, CDVDDemux* demuxer, CFileItemPtr part)
{
...
/* PLEX */
int COMXPlayer::GetStreamPlexID(StreamType type)
{
  int index = m_SelectionStreams.IndexOf(type, *this);
  if (index >= 0)
  {
    OMXSelectionStream stream = m_SelectionStreams.Get(type, index);
    return stream.plexID;
  }
  return -1;
}

void COMXPlayer::SetStreamPlexID(StreamType type, int plexID)
{
  if (plexID == -1)
    return;
  OMXSelectionStreams streams = m_SelectionStreams.Get(type);
  for (OMXSelectionStreams::iterator it = streams.begin(); it != streams.end(); ++it)
  {
